...
# comando de auto-py-to-exe [presento problemas con playwright]
# pyinstaller --noconfirm --onefile --console --icon "C:/comparadorV2/comparador/ui/resources/icons8-compare-60.ico" --paths "C:/Users/CECHEVARRIAMENDOZA/AppData/Local/ms-playwright/chromium-1124/chrome-win" --add-data "C:/comparadorV2/comparador/ui/resources/LOGO_NAVICO.png;." --add-data "C:/comparadorV2/comparador/ui/resources/style.qss;." --add-data "C:/comparadorV2/chrome.exe;." --paths "C:/comparadorV2/comparador/tools" --paths "C:/comparadorV2/comparador/ui/resources" --collect-data "playwright" --add-data "C:/Users/CECHEVARRIAMENDOZA/AppData/Local/ms-playwright/chromium-1124/chrome-win/chrome.exe;."  "C:/comparadorV2/comparador/app.py"

# ruta browser
# C:\Users\CECHEVARRIAMENDOZA\AppData\Local\Programs\Python\Python311\Lib\site-packages\playwright\driver\package\browsers.json
...
logo_path = os.path.abspath(r'comparador\ui\resources\LOGO_NAVICO.png')
...
class ComparadorApp(QMainWindow):
    ...
    def initUI(self):
        ...
        logo_navico.setPixmap(QPixmap(r"comparador\ui\resources\LOGO_NAVICO.png")) # carga de la imagen
        ...
        logo_navico2.setPixmap(QPixmap(r"comparador\ui\resources\LOGO_NAVICO.png")) # carga de la imagen
        ...
        logo_navico3.setPixmap(QPixmap(r"comparador\ui\resources\LOGO_NAVICO.png")) # carga de la imagen
    ...
    def comparar_archivos(self):
        ...
        if self.tabs.currentIndex() == 0:
            if self.ruta_bom.text() == "" or self.ruta_flexa.text() == "":
                ...
                return
            try:
                diferences_found,csv_path = comparador_pruebas.comparador(self.ruta_bom.text(),self.ruta_flexa.text())
        ...
        # si tab se encuentra en Nexxim
            ...
            if self.ruta_bom_nexxim.text() == "" or self.ruta_nexxim.text() == "":
                ...
                return
            try:
                diferences_found,csv_path =comparador_pruebas.comparacion_nexim(self.ruta_bom_nexxim.text(),self.ruta_nexxim.text())
        ...
        # si tab se encuentra en Bom
            ...
            if self.ruta_izq.text() == "" or self.ruta_der.text() == "":
                ...
                return
            try:
                diferences_found,csv_path =comparador_pruebas.comparacion_bom(self.ruta_izq.text(),self.ruta_der.text())
    ...
    def editar_archivo(self):
        # si tab se encuentra en Flexa
        if self.tabs.currentIndex() == 0:
            dlg = EditarDlg(parent=self,color="white", message="Archivo a editar?", text_boton_izq="Bom", text_boton_der="Flexa")
        ...
        # si tab se encuentra en Nexxim
            ...
            dlg = EditarDlg(parent=self,color="white", message="Archivo a editar?", text_boton_izq="Bom", text_boton_der="Nexxim")
            if dlg.exec() == QDialog.Accepted:
                resultado = dlg.obtener_resultado()
                if resultado == "Bom":
        ...
        # si tab se encuentra en Bom
            ...
            dlg = EditarDlg(parent=self,color="white", message="Archivo a editar?", text_boton_izq="Bom izq", text_boton_der="Bom der")
            if dlg.exec() == QDialog.Accepted:
                resultado = dlg.obtener_resultado()
    ...
    def nuevo_archivo(self):
        try:
            # crea un archivo excel en blanco en la ruta de downloads del usuario actual
            archivo = openpyxl.Workbook()
            nombre_archivo, _ = QInputDialog.getText(self, "Nombre del archivo", "Ensamble a comparar")
            
            if nombre_archivo:
                ruta_descargas = os.path.join(os.path.expanduser("~"), "Downloads")
                ruta_completa = os.path.join(ruta_descargas, f"{nombre_archivo}.xlsx")
                archivo.save(ruta_completa)
                if self.tabs.currentIndex() == 0:
                    self.ruta_flexa.setText(ruta_completa)
                elif self.tabs.currentIndex() == 1:
                    self.ruta_nexxim.setText(ruta_completa)
        except Exception as e:
            QMessageBox.critical(self, 'Error', f'Error: {e}')
    ...
    def guardar_devices_pdf(self):
        try:
            ...
            if file_dialog.exec():
                ...
                # Mostrar las listas categorizadas para depuración  ( para debug )
                ...
                for head_type in ['B', 'T']:
                    for unit_type in ['B', 'T']:
                        while head_files[head_type] and unit_files[unit_type]:
                            ...
                            #print(f"Base name: {base_name}")  # para debug
                            #base_name_suffix = f"{base_name}_{head_type}"
                            base_name_suffix = f'{base_name}_{head_type}'
                            #print(f"Base name suffix: {base_name_suffix}") # para debug
                            ...
                            #print(f"El archivo HTML '{base_name_suffix}' ha sido generado con éxito.") # para debug
                            report_number += 1
                
                self.convertir_a_pdf(devices)

        except Exception as e:
            logger.error(F"Error al procesar archivos XML: {e}")
            logger.info(f'----------------------------------------------\n')
            QMessageBox.error(self, "Error", "Error al procesar archivos XML. Verifique los archivos XML y vuelva a intentar.")
    ...
    def imprimir_devices(self):
        try:
            ...
            file_dialog.setDirectory("H:\\Publico\\SMT-Prog-Status\\DEVICES")  # ajustar ruta default de devices
            ...
            if file_dialog.exec():
                ...
                # ruta de la carpeta de los archivos seleccionados
                folder_path = os.path.dirname(selected_files[0])
                # nombre del archivo de salida
                output_filename = f"{os.path.basename(folder_path)}_devices.pdf"
                # Combinar la ruta de la carpeta con el nombre del archivo de salida y normalizarla
                salida_pdf = os.path.normpath(os.path.join(folder_path, output_filename))
                
                # Procesar cada PDF individualmente
                rutas_procesadas = []
                for archivo in selected_files:
                    texto_pdf = auto_print.leer_pdf(archivo)
                    modo_impresion = auto_print.determinar_impresion(texto_pdf)
                    ruta_final = auto_print.agregar_pagina_blanco(archivo, modo_impresion)
                    rutas_procesadas.append(ruta_final)
                # Fusionar PDFs procesados
                auto_print.fusionar_pdfs(rutas_procesadas, salida_pdf)
                # Imprimir el PDF fusionado en modo both side
                auto_print.imprimir_pdf(salida_pdf, "both side")
        except Exception as e:
            logger.error(f"Ocurrio un error al imprimir los PDFs: {str(e)}")
            logger.info(f'-------------------------------------------\n')
            QMessageBox.critical(self, "Error", f"Ocurrió un error al imprimir los PDFs: {str(e)}")
    ...
    async def convertir_html_a_pdf(self, files_to_convert):
        ...
        for i, file_path in enumerate(files_to_convert):
            ...
            QApplication.processEvents()  # Forzar actualización de la interfaz gráfica

            # Simulación de una pequeña pausa (opcional)
            await asyncio.sleep(0.5)
        
        # if progress_dialog.value() == progress_dialog.maximum():
        #     QMessageBox.information(self, "Finalizado", f"Se han convertido los archivos HTML a PDF.")
        QMessageBox.information(self, "Finalizado", f"Se han convertido los archivos HTML a PDF.")
        progress_dialog.close()

    async def url_to_pdf(self, url, output_path):
        try:
            # Usar la ruta relativa si el archivo está en la misma carpeta que el script
            #chrome_path = r'C:\Users\CECHEVARRIAMENDOZA\AppData\Local\ms-playwright\chromium-1124\chrome-win\chrome.exe'
            chrome_path = r'comparador\ui\resources\chrome-win\chrome.exe'
            async with async_playwright() as p:
                ...
                logger.info(f'Convertido {url} a {output_path}\n')
                logger.info(f'----------------------------------------\n')
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Ocurrio un error al convertir {url} a PDF: {str(e)}")
            logger.error(f'Error al convertir {url} a PDF: {str(e)}')
            logger.info(f'----------------------------------------\n')
            #print(f'Error al convertir {url} a PDF: {str(e)}')
...
class LogsDlg(QDialog):
    def __init__(self):
        super().__init__()
        self.setGeometry(450,450,550, 300)
        ...
        self.apply_format('DEBUG', QColor(Qt.darkYellow))
