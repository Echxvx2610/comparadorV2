...
def comparador(ruta_bom,ruta_flexa):    
    ...
    if "Yes" in placement['Skip'].values:
        ...
        logger.info(f"Se encontraron {len(skipeados)} componente(s) con skip, no.parte {skipeados['Part Number'].values} y referencia {skipeados['Reference'].values}")
    ...
    if placement['Assign'].isna().any():
        ...
        logger.info(f"Se encontraron {len(sin_asignar)} componente(s) sin asignar, no.parte {sin_asignar['Part Number'].values} y referencia {sin_asignar['Reference'].values}")
    ...
    comparacion_final = comparacion_final[~comparacion_final['Part Number'].str.startswith('Part')]
